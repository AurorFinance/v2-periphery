...
describe('ExampleComputeLiquidityValue', () => {
  ...
  describe('#getReservesAfterArbitrage', () => {
    it('1/400', async () => {
      ...
      expect(reserveA).to.eq('5000000000000000000')
      expect(reserveB).to.eq('2000000000000000000000')
    })
    it('1/200', async () => {
      const [reserveA, reserveB] = await computeLiquidityValue.getReservesAfterArbitrage(
        token0.address,
      ...
      expect(reserveA).to.eq('7071067811865475245')
      expect(reserveB).to.eq('1414213562373095048801')
    })
    ...
    it('1/50', async () => {
      ...
      expect(reserveA).to.eq('14142135623730950488')
      expect(reserveB).to.eq('707106781186547524402')
    })
    it('1/25', async () => {
      const [reserveA, reserveB] = await computeLiquidityValue.getReservesAfterArbitrage(
        token0.address,
      ...
      expect(reserveA).to.eq('20000000000000000000')
      expect(reserveB).to.eq('500000000000000000000')
    })
    it('25/1', async () => {
      const [reserveA, reserveB] = await computeLiquidityValue.getReservesAfterArbitrage(
        token0.address,
      ...
      expect(reserveA).to.eq('500000000000000000000')
      expect(reserveB).to.eq('20000000000000000000')
    })
    it('works with large numbers for the price', async () => {
      ...
      // diff of 30 bips
      expect(reserveA).to.eq('100000000000000000000')
      expect(reserveB).to.eq('100000000000000000000')
    })
  })
  ...
  describe('#getLiquidityValue', () => {
    describe('fee is off', () => {
      it('produces the correct value after arbing to 1:105', async () => {
        ...
        expect(token0Amount).to.eq('487950036474266589') // slightly less than 5% of 10, or 0.5
        expect(token1Amount).to.eq('51234753829797991916') // slightly more than 5% of 100, or 5
      })
      ...
      it('produces the correct value after arbing to 1:95', async () => {
        ...
        expect(token0Amount).to.eq('512989176042577047') // slightly more than 5% of 10, or 0.5
        expect(token1Amount).to.eq('48733971724044819536') // slightly less than 5% of 100, or 5
      })
      ...
      describe('after a swap', () => {
        beforeEach('swap to ~1:25', async () => {
          ...
          expect(reserve0).to.eq('19970000000000000000')
        ...
        it('is roughly 1/25th liquidity', async () => {
          ...
          expect(token1Amount).to.eq('25000000000000000000')
        })
